        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-md-6">
              ...
              <a href='addtable.php'><button type="button" class="btn btn-primary">Добавить таблицу</button></a>  
            </div>
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary card-header-icon">
                  ...
                  <h4 class="card-title">Поступившие заявки</h4>
                </div>
                <div class="card-body">
                  <div class="toolbar">
                  </div>
                  <div class="material-datatables">
                    <table id="datatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                      ...
                      <tbody>
                      <?php
                        $listRequest = mysqli_query($connect,"SELECT * FROM `request` ");
                        ...
                        <td><?=$row['№']?></td>
                          ...
                          <td><?=$row['Title']?></td>
                          <td><?=$row['Time']?></td>
                          <td><button class="btn btn-info" data-toggle="modal" data-target="#myModal" value="<?=$row['Thesis']?>" >Посмотреть</button></td>
                          <td><?=$row['Date']?></td>
                          <td>
                            <?php
                          ...
                          <td><?=$status["Title"]?></td>
                          <td class="text-right">
                          <?php include ('elements/selectionAdm.php');?>
                          </td>
                        </tr>
                        <?php endwhile;?>
                      </tbody>
            ...
            <?php
               $listtable = mysqli_query($connect,"SELECT * FROM `googleforms` ");
               while(($rowtable = mysqli_fetch_assoc($listtable)) != false) :
                $csv = file_get_contents($rowtable["url"]);
                $csv = explode("\r\n", $csv);
                $array = array_map('str_getcsv', $csv);
                file_put_contents(__DIR__ . '/list.json', json_encode($array));
                $array = json_decode(file_get_contents(__DIR__ . '/list.json'), true);
            ?>
            <div class="col-md-12">
              <div class="card ">
                ...
                <div class="card-body card ">
                  <div class="card-text">
                    <h4 class="card-title text-center">№ заявки - <?=$i?></h4>
                  </div>
                  <div class="row ">
                  <?php  foreach($array[$i] as $key => $value):?>
                    <div class="col-sm-4 mr-auto ml-auto">
                      <label class="col-sm-12 col-form-label text-center"><?=$array[0][$key]?></label>
